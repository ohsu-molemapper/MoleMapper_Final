...
- (void)awakeFromNib {
    ...
    NSNumber* total = [Mole30 moleCount];
    NSNumber* remaining = [NSNumber numberWithInt:[Mole30 numberOfMolesNeedingRemeasurement]];
    float numb = 0.0f;
    if ([total floatValue] > 0.0) {numb = (float)[remaining floatValue] / (float)[total floatValue];};
...
- (IBAction)showDescription:(UIButton *)sender {
    ...
    [[PopupManager sharedInstance] createPopupWithText:text completion:^{
