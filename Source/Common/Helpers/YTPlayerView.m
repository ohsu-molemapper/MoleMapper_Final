  ...
  if (ytMatch || adMatch || oauthMatch || staticProxyMatch || syndicationMatch) {
    return YES;
      ...
      [[UIApplication sharedApplication] openURL:url options:@{} completionHandler:^(BOOL success) {
          NSString *message = success ? @"Successfully opened" : @"Failed to open";
          NSLog(@"%@ URL: %@", message, url.absoluteString);
      }];
    return NO;
  }
}
...
- (BOOL)loadWithPlayerParams:(NSDictionary *)additionalPlayerParams {
  ...
  NSString *path = [[NSBundle mainBundle] pathForResource:@"YTPlayerView-iframe-player"
    ...
    // TCP EDITS
    path = [[NSBundle mainBundle] pathForResource:@"YTPlayerView-iframe-player.html" ofType:@"html"];
    // END EDITS
